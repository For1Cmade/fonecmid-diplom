...
Процедура СформироватьДвиженияПоОсновнымНачислениям() 
	...
	Запрос.Текст = 
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_СписокСотрудников.Сотрудник,
		...
		|			ГДЕ
		|				ВКМ_НачисленияЗарплатыОсновныеНачисления.Ссылка = &Ссылка
		|				И ВКМ_НачисленияЗарплатыОсновныеНачисления.ВидРасчета = &Оклад)) КАК ВКМ_УсловияОплатыСотрудниковСрезПоследних
		...
		|	ВКМ_НачисленияЗарплатыОсновныеНачисления.ГрафикРабот,
		|	ВЫБОР
		...
		|		ПО ВКМ_НачисленияЗарплатыОсновныеНачисления.Сотрудник = ВТ_Оклады.Сотрудник
		|ГДЕ
		|	ВКМ_НачисленияЗарплатыОсновныеНачисления.Ссылка = &Ссылка";
...
Процедура РассчитатьОтпускныеНачисления()
	...
	Запрос.Текст = 
		...
		|		(ВЫБРАТЬ
		|			BKM_СведенияОбОтпусках.Сотрудник
		|		ИЗ
		|			РегистрСведений.BKM_СведенияОбОтпусках КАК BKM_СведенияОбОтпусках
		|		ГДЕ
		|			BKM_СведенияОбОтпусках.Период = &ДатаОтсчета
		|			И BKM_СведенияОбОтпусках.Активность = ИСТИНА)";
	
	Запрос.УстановитьПараметр("Отпуск", ПланыВидовРасчета.ВКМ_ОсновныеНачисления.Отпуск);
	Запрос.УстановитьПараметр("ДатаОтсчета", ТекущаяДата()); 
...
Процедура СформироваДвиженияВыполненныеСотрудникомРаботы(Отказ)
	...
	Запрос.Текст = 
		...
		|	СУММА(ВКМ_НачисленияЗарплатыДополнительныеНачисления.СуммаНачисления) КАК СуммаНачисления,
		|	СУММА(ВКМ_НачисленияЗарплатыДополнительныеНачисления.НачисленоЧасов) КАК НачисленоЧасов
