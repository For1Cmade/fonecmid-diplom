...
КонецЕсли;
	...
	Если ЭтоНовый() Тогда 
		...
		ЗапросСправочник.Текст = "ВЫБРАТЬ
			...
			|ГДЕ
			|	ВКМ_УведомленияТелеграмБоту.Ссылка = &Ссылка";
		
		ЗапросСправочник.УстановитьПараметр("Ссылка", Ссылка);
		
		РезультатЗапроса = ЗапросСправочник.Выполнить();
		
		ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
		
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		
		ВыборкаДетальныеЗаписи.ТекстСообщения = "Создан новый документ Обслуживание Клиентов";
	...
	Сообщение2 = "Дата или время изменены";
	Если ДатаДокумента <> Дата Тогда 
		Выборка.ТекстСообщения = Сообщение2;
	КонецЕсли; 
	
	Сообщение3 = "Выбран Специалист";
	Если СпециалистДок <> Специалист Тогда 
		Выборка.ТекстСообщения = Сообщение3;
	КонецЕсли; 
