# Дипломный проект профессии «1C-программист: с нуля до middle» (Галкина Л.Р.)
...
Для работы с данным документом созданы предопределенные профили пользователей BKM_Специалист и BKM_Менеджер 
...
Выплаты осуществляются один раз в месяц после начисления зарплаты. Кроме этого на предприятии планируются отпуска на год вперёд.
...
2. Создать пользователей с правами доступа BKM_Бухгалтер, BKM_Кадровик-расчетчик, BKM_Специалист, BKM_Менеджер, выбрав соответствующий профиль пользователя

### Подсистема Обслуживание клиентов.
1. В режиме администратора настроить телеграмм-бот: https://github.com/For1Cmade/fonecmid-diplom/blob/v3/tasks/telegram.md
2. Настроить регламентное задание по отправке сообщений телеграмм-боту.
3. Из полученных данных настройки Телеграмм-бота заполнить Константы ВКМ_ТокенУправленияТелеграмБотом и ВКМ_ИдентификаторГруппыДляОповещения
...
2. Кадровик-расчетчик заполняет справочник Сотрудники фамилиями сотрудников, а так же заполняет регистр сведения УсловияОплатыСотрудников поле ПроцентОтВыполненныхРабот. Поля регистра сведения о графике работы сотрудника и окладу можно заполнить позже.

### Перед первым использованием подсистемы Расчет зарплаты.
1. Заполнить справочник графики работ. Для графика отпуск имеется предопределенная запись - Отпуск календарные дни. 
2. С помощью обработки Заполнение графиков работ, заполните соответствующий график. 
3. Кадровик-расчетчик в обязательном порядке должен дополнить регистр сведения УсловияОплатыСотрудников поля о графике работы сотрудника и окладу. 
