...
Запрос = Новый Запрос;
	Запрос.Текст = 
		...
		|	И (ДоговорыКонтрагентов.ВКМ_ПериодДействияНачало МЕЖДУ &ДатаНачалаПериода И &ДатаОкончанияПериода
		|				И (ДоговорыКонтрагентов.ВКМ_ПериодДействияКонец МЕЖДУ &ДатаНачалаПериода И &ДатаОкончанияПериода)
		|			ИЛИ ДоговорыКонтрагентов.ВКМ_ПериодДействияНачало <= &ДатаОкончанияПериода
		|				И ДоговорыКонтрагентов.ВКМ_ПериодДействияКонец >= &ДатаНачалаПериода)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ВЫБРАТЬ
		|	ВТ_ДействующиеДоговора.Договор КАК Договор,
		|	ВТ_СуществующиеДокументы.Документ КАК Документ,
	...
	ДоговорыРеализации = Новый ТаблицаЗначений;
	ДоговорыРеализации.Колонки.Добавить("Договор", Новый ОписаниеТипов("СправочникСсылка.ДоговорыКонтрагентов"));
	ДоговорыРеализации.Колонки.Добавить("Реализация", Новый ОписаниеТипов("ДокументСсылка.РеализацияТоваровУслуг"));	
	
	Пока Выборка.Следующий() Цикл                            
		
		ДокументРеализации = Выборка.Документ;
...
//		КонецЕсли;
		...
		Попытка 
			Если ДокументРеализации.ПроверитьЗаполнение() Тогда 
				...
				Строка = ДоговорыРеализации.Добавить();
	...
	Результат.Вставить("МассивДоговоровИРеализаций", ОбщегоНазначения.ТаблицаЗначенийВМассив(ДоговорыРеализации));
...
//		|	НЕ ДоговорыКонтрагентов.ПометкаУдаления
//		|	И ДоговорыКонтрагентов.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.ВКМ_АбонентскоеОбслуживание)
//		|	И ДоговорыКонтрагентов.ВКМ_ПериодДействияНачало <= &ДатаОкончанияПериода
//		|	И ДоговорыКонтрагентов.ВКМ_ПериодДействияКонец >= &ДатаНачалаПериода
//		|;
//		|
//		|////////////////////////////////////////////////////////////////////////////////
...
//		|	ВТ_ДействующиеДоговора.ДоговорПредставление,
//		|	ВТ_ДействующиеДоговора.КонтрагентПредставление
//		|ИЗ
//		|	ВТ_ДействующиеДоговора КАК ВТ_ДействующиеДоговора
//		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СуществующиеДокументы КАК ВТ_СуществующиеДокументы
//		|		ПО ВТ_ДействующиеДоговора.Договор = ВТ_СуществующиеДокументы.Документ
//		|ГДЕ
...
//		
//		ДокументРеализации = Выборка.Документ;
//		Если ДокументРеализации = null Тогда
//		    ДокументРеализации = Документы.РеализацияТоваровУслуг.СоздатьДокумент();
//		КонецЕсли;
//
//	    ДокументРеализации.Заполнить(Выборка);
